...
const FeedbackCarousel = () => {
  ...
  useEffect(() => {
    const checkScreen = () => setIsMobile(window.innerWidth < 1024); // < 1024px is mobile/tablet
    checkScreen();
    window.addEventListener('resize', checkScreen);
    return () => window.removeEventListener('resize', checkScreen);
  }, []);

  // Swipe handlers for mobile/tablet only
  ...
  return (
    ...
    >
      ...
      <div
        ...
        {...(isMobile ? swipeHandlers : {})} // Apply swipe only on mobile/tablet
      >
        ...
        <div className="max-w-full sm:max-w-3xl px-4 sm:px-0 mb-4" style={{ minHeight: '220px' }}>
          <p className="text-sm sm:text-base md:text-lg leading-relaxed">{text}</p>
        </div>
